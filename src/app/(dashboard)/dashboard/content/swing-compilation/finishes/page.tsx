...
const SYSTEM_KEY = 'discover:swing-compilation:finishes';
  ...
  return (
    <main className="mx-auto max-w-5xl text-white">
      <section className="mb-8">
        ...
        <h1 className="mt-3 text-4xl font-extrabold">Finishes</h1>
        <p className="mt-3 max-w-2xl text-zinc-400">
          Browse finish positions and end-pattern examples.
        </p>
      </section>

      {items.length === 0 ? (
        <section className="rounded-[24px] border border-white/10 bg-white/[0.04] p-6">
          <p className="text-lg font-semibold text-white">No finish content yet</p>
          <p className="mt-2 text-sm text-zinc-400">
            Add discover-routed content with the Finishes subcategory in admin.
          </p>
        </section>
      ) : (
